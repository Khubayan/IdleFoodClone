...
func _process(delta: float) -> void:
	current_coins.text = str(GameManager.format_coins(GameManager.current_coins))
...
func _ready() -> void:
	...
	faster_coffee.text = GameManager.format_coins(faster_cofee_cost)
...
func _on_faster_coffee_pressed() -> void:
	SoundManager.play_ui()
	if GameManager.current_coins >= faster_cofee_cost:
		GameManager.current_coins -= faster_cofee_cost
